...
This has been shown, for example, in Scontras et al. (2017). This team of researchers found\idx{sound} out that Mandarin lacks \textit{inverse scope}. Inverse scope can best be explained by an example: \lq\lq A badger dug every hole \rq\rq. In English, two readings are available for such sentence:
...
\noindent Scontras et al. found\idx{sound} out that the inverse scope reading is simply not available in Mandarin Chinese. Furthermore, they found\idx{sound} out this lack of inverse scope is found\idx{sound} in the English of native Mandarin speakers. Another study showed similar results: Korean\idx{Korean} learners of English habitually preferred the surface-scope reading, and left the inverse-scope reading out (Seon \& Shin, 2022). 
...
Of course, this was a fabricated example. A real-life example, can be found\idx{sound} in (Layman,1962):
...
\noindent is undeniably witty\footnote{Found\idx{sound} in (Cornish-Bowden, 2015).}. Does the fruit fly similar to a banana, or do fruit flies love a banana? The first part of (5) seems to prime the reader for the first reading.  
...
\section{Atlan's grammar\idx{grammar}}
...
\section{Greenberg\idx{Greenberg}'s universals}
...
\item +  \lq\lq With well more than chance frequency, when question particles or affixes are specified in position by reference to the sentence as a whole, if initial, such elements are found\idx{sound} in prepositional languages, and, if final, in postpositional." 
...
\item +  \lq\lq When any or all of the items (demonstrative, numeral,\idx{numerals} and descriptive adjective) precede the noun, they are always found\idx{sound} in that order. If they follow, the order is either the same or its exact opposite." 
