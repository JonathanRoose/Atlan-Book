...
\noindent Let's dissect this letter. This is the letter \lq\lq mok \rq\rq, or ([/mok/]) phonetically. First step is to discover the main line, which is the long diagonal here. This is the consonant {\it m} (/m/). Then there is a smaller line, found  in the bottom-right corner. This is the {\it k} ([/k/]). The horizontal line is in the bottom-right of our imaginary square. Hence, the {\it m} comes before the {\it k} (see also figure one). We got the two consonants\idx{consonants}, now rests the vowel. Feel free to look back at figure two. The smaller line is found  in the bottom-right corner, hence the vowel here is an {\it o} ([/o/]). The full syllable\idx{syllable} is {\it mok} ([/mok/]). 
...
\vspace{0.2cm}
Now let's look at another one. See if you can determine the syllable\idx{syllable} yourself first. The main line is obvious: it's the big curve. This big curve is a {\it j\footnotemark} ([/j/]). The smaller dot is a {\it p} ([/p/]). The dot is found  inside the quadrant \lq\lq UL\rq\rq of figure one. Hence, the dot comes first. The dot is found  a bit to the left of the centre of our imaginary box. Hence, the vowel here, is the {\it a} ([/a/]). The full syllable\idx{syllable} is {\it jap} ([/jap/]). 
...
\restoregeometry
\section{Direction of reading\idx{direction of reading} -- {\small Stijn Janssens}}
...
\section{Punctuation\idx{punctuation} -- {\small Stijn Janssens}}
Atlan has minimal punctuation\idx{punctuation}, only having dedicated symbols for a comma and a full stop, and spaces are the same as in any other orthography\idx{orthography}. A comma is notated as a small half circle which is open at the top:\comma symbolizing an ‘open’ continuation of the sentence, and the full stop is notated at as a half circle which is open at the bottom: the \period , symbolizes a closed sentence. 
...
\section{Transliteration\idx{transliteration} -- {\small Stijn Janssens}}
...
\section{Numerals\idx{numerals} -- {\small Stijn Janssens}}
...
\section{Mathematics\idx{mathematics}-- {\small Stijn Janssens}}
...
\section{Typography -- {\small Stijn Janssens}}
...
\end{center}
\section{Names, loanword\idx{loanword}s, and cartouche\idx{cartouche}s -- {\small Stijn Janssens}}
...
In (formal) typography, a cartouche\idx{cartouche} may be employed to encircle the phonetic approximation in order to enhance intelligibility. Cartouche\idx{cartouche}s originate from Ancient Egyptian hieroglyphic orthography\idx{orthography}, where they were used to encircle the names of pharaohs (see Fig. 7) (Chrisholm, 1911).  

\begin{wrapfigure}{l}{0.4\textwidth}
\includegraphics[scale=0.1]{./Images/Cartouche.jpeg}

	{\footnotesize \it Fig.7 Cartouche\idx{cartouche} of Hapshepsut, one of the few female pharaohs in Egyptian history. She was often referred to androgynously because of a lack of feminine royal nomenclature (Graves-Brown, 2010). }
...
Then, of course, this book is typeset in \TeX\idx{TeX} . We could have made it easier for ourselves. But, typesetting with \TeX\idx{TeX}\ was worth the effort. We are proud of what we have made; both content- and appearance-wise. Code can be found on our website\footnote{https://www.Atlanial.com}.    
