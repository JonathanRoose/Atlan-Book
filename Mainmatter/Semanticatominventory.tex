...
\begin{center}
	...
	\hline
		...
		\thelexi.\stepcounter{lexi} Relative clause & I$^{+pronoun}$ \newline & \Atlani \\	
	\hline
	\end{tabular}
}
	\end{center}
...
\begin{center}
	...
	\hline
		...
		\tabb{Intention} & UF & \uf\\
		\tabb{Cause, reason  \newline (effect = caused, passive)} & KO & \ko\\
...
\resizebox{1\textwidth}{!}{
	...
	\thelexi.\stepcounter{lexi} Love & LOF & \textbackslash lof \\
	\thelexi.\stepcounter{lexi} & KEF & \textbackslash kef \\
	\thelexi.\stepcounter{lexi} & LEJ & \textbackslash lej \\
	\hline
	\end{tabular}
	}
\end{center}
...
\resizebox{1\textwidth}{!}{
	...
	\hline
	\thelexi.\stepcounter{lexi} Consciousness, mind & MAK & \mak \\
	...
	\thelexi.\stepcounter{lexi} Stimulation, excitation (negative = sedation) & JIS & \jis \\
...
\resizebox{1\textwidth}{!}{
	\begin{tabular}{|p{0.6\textwidth} | cx{0.2\textwidth}|cx{0.2\textwidth}|}
		\hline
		\multicolumn{3}{|c|}{\bf CONSCIOUSNESS (Continued)}\\
	\hline
...
\resizebox{1\textwidth}{!}{
	...
	\thelexi.\stepcounter{lexi} Axis & KEJ & \textbackslash kej \\
	\thelexi.\stepcounter{lexi} & POP & \pop \\
	\thelexi.\stepcounter{lexi} & LUJ & \luj \\
	\hline
	\end{tabular}
	}
\end{center}
...
\resizebox{1\textwidth}{!}{
	...
	\hline
	XXX Nature & NIF & \textbackslash nif \\
	...
	\thelexi.\stepcounter{lexi} Ecosystem, biome (+ tree = forest) & PIM & \pim \\
	\thelexi.\stepcounter{lexi} Stick, branch & FAP & \fap \\
	\thelexi.\stepcounter{lexi} Fruit (+ fungus = mushroom) & FUT & \fut \\
	\thelexi.\stepcounter{lexi} Seed (+ fungus = spore, + animal = sperm) & NIP & \nip \\
	\thelexi.\stepcounter{lexi} Leaf & LIF & \lif \\
	\thelexi.\stepcounter{lexi} Root (+ fungus = mycelium, + shape = rhizome) & LUT & \lut \\
...
\hline
\tabb{	Consume (+ solid = eat, + liquid = drink)  }
&
KOS 
...
\pul 
\\
\tabb{	Breathe (+ organ = lung, + smoke = to smoke something)  }
...
\resizebox{1\textwidth}{!}{
	...
	\thelexi.\stepcounter{lexi} Slime, mucus (+ mouth = saliva, + nose = snot) & LIL & \lil \\
	...
	\thelexi.\stepcounter{lexi} Organ (+ hear = ear, + see = eye, + smell = nose, + taste = tongue) & NOK & \nok \\
...
\resizebox{1\textwidth}{!}{
	...
	\thelexi.\stepcounter{lexi} Joint (+ leg = knee, + arm = elbow, + line = angle) & TUJ & \tuj \\
	...
	\thelexi.\stepcounter{lexi} Waste (+ solid = faeces, + liquid = urine) & FES & \fes \\
	\hline
	\end{tabular}
	}
\end{center}
...
\sek 
\\
\tabb{	Birth (+ active = to birth, + passive = to be born) }
...
\begin{tabular}{|p{0.6\textwidth} | cx{0.2\textwidth} | cx{0.2\textwidth}|}
\hline
	\multicolumn{3}{|c|}{{\bf SUBSTANCE (Continued)}}\\
\hline
...
\tabb{	Orbital system (+ sun = solar system, + star = galaxy) } &
...
\end{tabular}
\footnotemark
\footnotetext{Not all syllables were used, since we have 490 options in total, there are 42 additional words which weren't used and can be used when additional words are added when expanding the language.}
}
\end{center}
\endgroup
\vfill
